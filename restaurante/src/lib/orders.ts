...
export function useIncomingOrders(restaurantId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    setOrders(
      ((data ?? []) as unknown as RawOrder[]).map((order) => ({
        id: order.id,
  ...
  // leitura que a query acima já usa. INSERT é tratado separado de UPDATE
  // porque só pedido novo deve disparar o som de notificação.
  const [justInsertedOrderId, setJustInsertedOrderId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setJustInsertedOrderId((payload.new as { id: string }).id);
          load();
        },
      )
      .on(
  ...
  return {
    ...
    registerPayment,
    justInsertedOrderId,
    clearJustInsertedOrder: () => setJustInsertedOrderId(null),
  };
}
