...
PDV Agência Sigma — a multi-tenant restaurant ordering platform. One agency (ADM) manages many restaurant tenants, each with their own menu, tables, and orders; end customers order through a public storefront. All user-facing text, SQL comments, and code comments are in Portuguese (pt-BR) — keep new UI copy and comments in Portuguese to match. `PDV.md` (pt-BR) is a narrative walkthrough of the product screen-by-screen; it documents an earlier iteration of the ordering flow (per-table QR codes, `table_sessions` comandas) that has since been replaced — see "Ordering flow" below for the current model. Treat `PDV.md` as historical context, not ground truth.
